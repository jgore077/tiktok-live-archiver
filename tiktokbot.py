...
class TikTok:
    ...
    def run(self):
        if not os.path.exists('lives'):
            os.mkdir('lives')
        # I will need a way to join fragments of livees together
        """
    ...
    def start_recording(self):
        ...
        try:
            if self.use_ffmpeg:
                ...
                filename=output.replace("_flv.mp4", ".mp4")
                if self.duration is not None:
                    stream = ffmpeg.output(stream,filename, c='copy', t=self.duration)
                else:
        ...
        if not self.convert:
            ...
            self.convertion_mp4(output)
        # I think I should upload the video here
